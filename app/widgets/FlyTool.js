...
define([
  ...
  const FlyTool = Accessor.createSubclass([Evented], {
    ...
    _createButton: function () {
      ...
      const flyToolBtn = domConstruct.create("button", { className: "fly-btn btn", title: "Fly" });
      domConstruct.create("img", { src: this.resources.image_16 }, flyToolBtn);

      on(flyToolBtn, "click", () => {
        this.enabled = (!this.enabled);
        domClass.toggle(flyToolBtn, "btn-clear", this.enabled);
        //domStyle.set(flyToolBtn, "backgroundColor", this.enabled ? "#fff !important" : "#0079c1");
      });
    ...
    _fly: function () {
      if(this.flying && this.mouseMovePoint) {
         ...
         */
        const distanceOffset = Math.abs(camera.position.z * this.moveSpeed.FACTOR);
        // XY OFFSETS //
         ...
         */
        if(this.view.map.ground.navigationConstraint.type !== "none") {
